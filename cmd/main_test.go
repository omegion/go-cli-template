...
func TestCommander_SetRootCommand(t *testing.T) {
	...
	assert.Equal(t, "go-cli", commander.Root.Use)
	assert.Equal(t, "Go CLI application template", commander.Root.Short)
	assert.Equal(t, "Go CLI application template for Go projects.", commander.Root.Long)
